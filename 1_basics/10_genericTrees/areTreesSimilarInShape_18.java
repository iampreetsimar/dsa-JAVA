// 1. Given GenericTree class, complete areSimilar function. 
// The function is passed two GT roots and expected to return true if both trees are similar, else false.
...
// Output
// true -> tree1 and tree2 are similar

import java.util.Stack;
import java.util.ArrayList;

public class areTreesSimilarInShape_18 {
    public static void main(String[] args) {
        ...
        System.out.println(tree1.areSimilar(tree1.root, tree2.root));
    }
    ...
    public static class GenericTree {
        ...
        boolean areSimilar(Node node1, Node node2) {
            // BASE CASE : on reaching a node, check node1(tree1) and node2(tree2) number of children
            // if different -> not similar in shape -> return false -> control does not move forward
            if(node1.children.size() != node2.children.size())
                return false;

            // node1 and node2 have same number of children
            // recursive calls to all children
            // if result from any of the child is false -> that child1 and child2 at that point are not similar
            ...
            // children of node1 and that of node2 are also similar
            // return true -> everything from node1 and node2 onwards is similar
            return true;
        }
    }
}
