...
public class knightsTour_4 {
    ...
    // upcoming move - move no of knight
    public static void printKnightsTour(int[][] chess, int r, int c, int upcomingMove) {
        if(r < 0 || c < 0 || r >= chess.length || c >= chess.length || chess[r][c] != 0)
            return;     // INVALID CASE - out of bounds OR cell already visited

        if(upcomingMove == chess.length * chess.length) {
            // BASE CASE
            // since this is the last move, it is not yet marked in the cell
            // mark the cell with move no, display the board and unmark while leaving the cell
        ...
        // mark cell while visiting
        chess[r][c] = upcomingMove;

        // move in all 8 possible directions and update the move no as well
        ...
        // unmark cell while leaving so that cell can be used for some other config
        chess[r][c] = 0;
    }
