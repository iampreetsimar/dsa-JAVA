...
public class floodFill_1 {
    ...
    public static void floodfill(int[][] maze, int sr, int sc, String psf, boolean[][] visited) {
        if(sr < 0 || sr == maze.length || sc < 0 || sc == maze.length)
            return;     // OUT of BOUNDS

        if(maze[sr][sc] == 1 || visited[sr][sc] == true)
            return;     // Obstacle present or already visited
        ...
        // on reaching a cell, mark it visited
        visited[sr][sc] = true;

        // move in all directions to explore all paths and update the path solution as well
        // top -> left -> down -> right
